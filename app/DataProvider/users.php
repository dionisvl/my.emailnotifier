 ...
 */
function getExpiresUsersWithGoodValidatedEmail(PDO $DBH): array
{
    return $DBH
        ->query("
        SELECT * FROM users 
...
function getExpiresUsersWithNotValidatedEmail(PDO $DBH): array
{
    return $DBH
        ->query("
        SELECT * FROM users 
...
function confirmUser(PDO $DBH, int $userId): void
{
